...
#sz <- 1:n/n+10
sz <- (1:n/n)*100+10
input_var <- expand.grid("n" = n, "m" = m, "p" = p, "beta" = beta, "size" = sz)
input_var
```
...
```{r}
random_graph <- function(n, power, m, beta, size) {
      g1 <- igraph::sample_pa(n = size, power = power, m = m, directed = TRUE)
