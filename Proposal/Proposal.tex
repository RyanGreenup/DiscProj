...
\lstset{language=julia,label=eigen-julia,caption={Time to Solve Eigen Value for matrix of size \(n\)},captionpos=b,numbers=none}
\begin{lstlisting}
using LinearAlgebra
using TimerOutputs
...
\lstset{language=r,label=random-surf-r,caption={Implementing the random surfer model for the graph shown in figure },captionpos=b,numbers=none}
\begin{lstlisting}
library(igraph)
library(tidyverse)
...
\lstset{language=r,label=pure-math-adj,caption={\emph{PageRank} Probability Transition Matrix of network based on divisibility of \(\mathbb{Z}^+ \leq 30\)},captionpos=b,numbers=none}
\begin{lstlisting}
library(igraph)
library(tidyverse)
